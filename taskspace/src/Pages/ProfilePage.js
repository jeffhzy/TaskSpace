...
const ProfilePage = (props) => {
    ...
    useEffect(() => {
        getDownloadURL(ref(storage, `${userUid}/profilepic`))
            .then((url) => {
                setImageURL(url);
            })
        const getStartInfo = async () => {
            const Data = await getDoc(doc(db, "users", userUid));
            setUserProfile({ name: (Data.data().firstName + " " + Data.data().lastName), level: Math.floor(Data.data().points / 300) + 1 });
            const friendCheck = (Data.data().friends.filter((i) => i === user.uid).length > 0) ? true : false;
            const requestCheck = (Data.data().requests.filter((i) => i === user.uid).length > 0) ? "request" : false;
            friendCheck ? setAdded(true) : setAdded(requestCheck);
            if (Data.data().requests.filter((r) => r === user.uid)[0] === user.uid) { setAdded('sent'); };
        };
        getStartInfo();
    ...
    const addFriend = async () => {
        ...
        await setDoc(doc(db, "users", props.id), { friends: friends2 }, { merge: true });
    }
    ...
    const removeFriend = async () => {
        ...
        await setDoc(doc(db, "users", user.uid), { friends: friend1 }, { merge: true });
        await setDoc(doc(db, "users", props.id), { friends: friend2 }, { merge: true });
    }

    const requestHandler = () => {
        if (added === false) {
            const sendRequest = async () => {
                await setDoc(doc(db, "users", userUid), { requests: [user.uid] }, { merge: true });
            };
            sendRequest();
            setAdded('sent');
    ...
    return (
        <>
            <Header />
            <div className="profile-pageOrientation">
                <Sidebar className="profile-header-sidebar" values={{}} />
                <div className="profile-mainPart">
                    <img src={imageURL} alt={Anon} className="profile-icon"></img>
                    <label>{userProfile.name}</label>
                    <label style={{ marginTop: '0px', fontSize: '25px' }}>Level {userProfile.level}</label>
                    {added === null ? <></> : <Button variant="outlined" color="primary" onClick={requestHandler}>{added === false ?
                        "Send Friend Request" : (added === "sent") ? `Friend Request Sent` :
                            (added === "request") ? "Accept Friend Request" : "Remove Friend"}</Button>}
                </div>
            </div>
        </>
